...
void main() {
  test('PASS 346E Firefly uses the actual singular runtime filename', () {
    expect(
      ArcItemAssetRegistry.assetPathForId('firefly-burner'),
      'assets/arc_raiders/scrappy_resources/firefly_burner.webp',
    );
    expect(
      File(ArcItemAssetRegistry.assetPathForId('firefly-burner')).existsSync(),
      isTrue,
    );
  });

  test('Scrappy remains const-safe and registry-based', () {
    final source = File(
    ...
    for (final item in ArcScrappySeedData.items.whereType<ArcScrappyItem>()) {
      expect(item.imageAsset, startsWith(ArcItemAssetRegistry.runtimeBase));
    }
  });

  test('every Scrappy Bench Quest runtime asset exists', () {
    final paths = <String>{
  ...
  test('reference scrape artifacts remain retained outside runtime assets', () {
    const names = <String>[
