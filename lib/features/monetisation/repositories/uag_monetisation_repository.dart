...
class UagMonetisationRepository {
  ...
  Stream<UagEntitlement> watchMyEntitlement() {
    ...
    return _firestore.collection('users').doc(uid).snapshots().map((snapshot) {
      return UagEntitlement.fromUserDoc(
        uid,
  ...
  Stream<UagUsageSnapshot> watchMyWeeklyUsage() {
    ...
    return _firestore
        ...
        .snapshots()
        .map(
          (snapshot) => UagUsageSnapshot.fromMap(
            snapshot.data() ?? const <String, dynamic>{},
          ),
        );
  }
  ...
  Stream<List<UagImpactPotSnapshot>> watchImpactPots() {
    return _firestore
        .collection('impact_pots')
        .orderBy('sortOrder')
        .snapshots()
        .map(
          (snapshot) => snapshot.docs
              .map(UagImpactPotSnapshot.fromDoc)
              .toList(growable: false),
        );
  }

  Stream<QuerySnapshot<Map<String, dynamic>>> watchAdminRevenueEvents({
    int limit = 50,
  }) {
    return _firestore
  ...
  Future<void> ensureReferralCode(String code) async {
    final uid = currentUid;
    if (uid == null) return;
    final normalised = code.trim().toUpperCase().replaceAll(
      RegExp(r'[^A-Z0-9]'),
      '',
    );
