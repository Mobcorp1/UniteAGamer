...
class AdminMonetisationDashboard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text('Monetisation', style: AppTheme.tradingHeading(fontSize: 26, color: AppTheme.neonCyan)),
          ...
          builder: (context, snapshot) {
            ...
            return LayoutBuilder(
              builder: (context, constraints) {
                ...
                final cards = [
                  _StatCard(label: 'Free Users', value: '${stats.freeUsers}', color: Colors.white70),
                  _StatCard(label: 'Essential Users', value: '${stats.essentialUsers}', color: AppTheme.neonCyan),
                  _StatCard(label: 'Premium Users', value: '${stats.premiumUsers}', color: AppTheme.neonPink),
                  _StatCard(label: 'Admin / Dev', value: '${stats.adminUsers}', color: AppTheme.warningAmber),
                ];
                if (!isWide) {
        ...
        StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snapshot) {
            ...
            return Container(
              ...
              decoration: AppTheme.tradingCardDecoration(borderColor: AppTheme.neonCyan.withValues(alpha: 0.22)),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Revenue Snapshot', style: AppTheme.tradingHeading(fontSize: 22, color: AppTheme.neonCyan)),
                  const SizedBox(height: AppTheme.spaceM),
                  _moneyRow('Gross paid revenue logged', stats.grossPence),
                  _moneyRow('Estimated Stripe fees', stats.stripeFeesPence),
                  _moneyRow('Referral commission owed', stats.referralCommissionPence),
                  _moneyRow('Net platform profit logged', stats.netPlatformProfitPence),
  ...
  Widget _moneyRow(String label, int pence) {
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(child: Text(label, style: AppTheme.bodyTextStyle(fontSize: 14, color: Colors.white70))),
          Text('£${(pence / 100).toStringAsFixed(2)}', style: AppTheme.bodyTextStyle(fontSize: 14, color: Colors.white, isBold: true)),
        ],
      ),
    );
  }
...
class _PaymentMethodCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      padding: AppTheme.sectionCardPadding,
      decoration: AppTheme.tradingCardDecoration(borderColor: AppTheme.warningAmber.withValues(alpha: 0.22)),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Payment Channels', style: AppTheme.tradingHeading(fontSize: 22, color: AppTheme.warningAmber)),
          const SizedBox(height: AppTheme.spaceS),
          Text(
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(label, style: AppTheme.bodyTextStyle(fontSize: 13, color: AppTheme.tradingMutedText, isBold: true)),
          const SizedBox(height: 6),
          Text(value, style: AppTheme.tradingHeading(fontSize: 28, color: color)),
        ],
      ),
    );
  }
...
class _AdminUserStats {
  const _AdminUserStats({required this.freeUsers, required this.essentialUsers, required this.premiumUsers, required this.adminUsers});
  ...
  factory _AdminUserStats.fromDocs(List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) {
    var free = 0;
    var essential = 0;
    var premium = 0;
    var admin = 0;
    ...
    return _AdminUserStats(freeUsers: free, essentialUsers: essential, premiumUsers: premium, adminUsers: admin);
  }
}

class _RevenueStats {
  const _RevenueStats({required this.grossPence, required this.stripeFeesPence, required this.referralCommissionPence, required this.netPlatformProfitPence, required this.charityPence});
  ...
  factory _RevenueStats.fromDocs(List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) {
    var gross = 0;
    var stripe = 0;
    var referral = 0;
    var net = 0;
    ...
    return _RevenueStats(grossPence: gross, stripeFeesPence: stripe, referralCommissionPence: referral, netPlatformProfitPence: net, charityPence: charity);
  }
}
