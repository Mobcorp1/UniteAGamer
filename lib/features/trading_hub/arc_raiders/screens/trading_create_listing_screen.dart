...
class _TradingCreateListingScreenState extends State<TradingCreateListingScreen> {
  ...
  List<ArcBlueprint> get _dupeBlueprints => _blueprints.where((blueprint) {
        final state = _states[blueprint.id] ?? ArcBlueprintState.empty(blueprint.id);
        return state.hasDuplicates;
      }).toList(growable: false);

  List<ArcBlueprint> get _missingBlueprints => _blueprints.where((blueprint) {
        final state = _states[blueprint.id] ?? ArcBlueprintState.empty(blueprint.id);
        return !state.owned;
      }).toList(growable: false);

  int get _seedTotal => (_smallBundles * 10) + (_mediumBundles * 50) + (_largeBundles * 100);

  bool get _hasAnyOfferedSelection =>
      _selectedOfferingBlueprints.isNotEmpty || _selectedOfferingAssets.isNotEmpty || _seedTotal > 0;
  ...
  Widget _sectionCard({required String title, String? subtitle, required Widget child}) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(title, style: AppTheme.tradingHeading(fontSize: 21, color: AppTheme.neonPink)),
          if (subtitle != null && subtitle.isNotEmpty) ...[
            const SizedBox(height: 6),
            Text(subtitle, style: TextStyle(color: AppTheme.tradingMutedText, height: 1.3)),
          ],
  ...
  Widget _bundleRow({
    ...
    return Row(
      children: [
        ...
        Text('$value', style: AppTheme.tradingHeading(fontSize: 22, color: AppTheme.neonCyan)),
        IconButton(
          onPressed: onPlus,
        ...
        child: Column(
          ...
          children: [
            ...
            if (helper != null && helper.isNotEmpty) ...[
              const SizedBox(height: 6),
              Text(helper, style: TextStyle(color: AppTheme.tradingFaintText, fontSize: 12)),
            ],
          ],
        ),
      ),
      ...
      children: items
          .map(
            (item) => Container(
              ...
              decoration: AppTheme.tradingPillDecoration(color: color ?? AppTheme.neonCyan),
              child: Text(
                item,
                style: TextStyle(
            ...
            return SafeArea(
              child: Padding(
                ...
                child: SizedBox(
                  ...
                  child: Column(
                    children: [
                      Text(title, style: AppTheme.tradingHeading(fontSize: 22, color: AppTheme.neonCyan)),
                      const SizedBox(height: AppTheme.spaceM),
                      TextField(
                        controller: controller,
                        style: const TextStyle(color: Colors.white),
                        onChanged: updateFilter,
                        decoration: AppTheme.tradingInputDecoration(label: 'Search blueprints'),
                      ),
                      const SizedBox(height: AppTheme.spaceM),
                      Align(
                      ...
                      Expanded(
                        child: filtered.isEmpty
                            ? const Center(
                                child: Text('No matching blueprints.', style: TextStyle(color: Colors.white60)),
                              )
                            : ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return CheckboxListTile(
                                    ...
                                    title: Text(blueprint.name, style: const TextStyle(color: Colors.white)),
                                    subtitle: Text(
                                      state.hasDuplicates
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: () {
                            final selected = items.where((item) => selectedIds.contains(item.id)).toList(growable: false);
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      TextField(
                        ...
                        decoration: AppTheme.tradingInputDecoration(label: 'Search weapons, keys, mods, materials'),
                      ),
                      const SizedBox(height: AppTheme.spaceM),
                      ...
                      Expanded(
                        child: filtered.isEmpty
                            ? const Center(
                                child: Text('No matching trade assets.', style: TextStyle(color: Colors.white60)),
                              )
                            : ListView.builder(
                                itemCount: filtered.length,
                                itemBuilder: (context, index) {
                                      ...
                                      children: [
                                        ...
                                        if (item.tradeValue == ArcTradeValueTier.elite || item.rarity == ArcTradeItemRarity.legendary)
                                          Container(
                                            ...
                                            child: const Text(
                                              'HOT',
                                              style: TextStyle(color: AppTheme.neonPink, fontWeight: FontWeight.w800, fontSize: 11),
                                            ),
                                          ),
                                      ],
                                    ),
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: () {
                            final selected = _tradeCatalog.where((item) => selectedIds.contains(item.id)).toList(growable: false);
                            Navigator.of(context).pop(selected);
                          },
                          style: ElevatedButton.styleFrom(
                            backgroundColor: AppTheme.neonPink,
  ...
  Future<void> _saveListing() async {
    if (!_hasAnyOfferedSelection) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Add at least one offered blueprint, item, or seed bundle.')),
      );
      return;
    }
    ...
    try {
      await _repository.createListing(
        ...
        offeredBlueprintNames: _selectedOfferingBlueprints.map((item) => item.name).toList(growable: false),
  ...
  Widget _profilePrefillCard(TradingProfile profile) {
    return _sectionCard(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Trader: ${profile.displayName}', style: const TextStyle(color: Colors.white)),
          const SizedBox(height: 8),
          Text('Region: ${profile.region}', style: TextStyle(color: AppTheme.tradingMutedText)),
          const SizedBox(height: 8),
          Text(
          ...
          SafeArea(
            child: StreamBuilder<Map<String, ArcBlueprintState>>(
              ...
              builder: (context, stateSnapshot) {
                _states = stateSnapshot.data ?? const <String, ArcBlueprintState>{};
                return FutureBuilder<TradingProfile>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    final profile = snapshot.data ?? TradingProfile.empty(_repository.currentUid ?? '');
                        ...
                        child: ListView(
                          ...
                          children: [
                            ...
                            _sectionCard(
                              ...
                              subtitle: 'Build a trade package with multiple blueprints, weapons, keys, mods, resources, and seeds.',
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  _selectorTile(
                                    label: 'Offering Blueprints',
                                    value: _selectionSummary(_selectedOfferingBlueprints.length, 'blueprint'),
                                    ...
                                    onTap: dupeBlueprints.isEmpty
                                        ...
                                        : () async {
                                            final picked = await _showBlueprintMultiPicker(
                                              title: 'Select offering blueprints',
                                              items: dupeBlueprints,
                                              initiallySelected: _selectedOfferingBlueprints,
                                            );
                                  ...
                                  const SizedBox(height: AppTheme.spaceS),
                                  _chipWrap(_selectedOfferingBlueprints.map((item) => item.name).toList(growable: false)),
                                  const SizedBox(height: AppTheme.spaceM),
                                  _selectorTile(
                                    ...
                                    onTap: () async {
                                      final picked = await _showAssetMultiPicker(
                                        ...
                                        initiallySelected: _selectedOfferingAssets,
                                      );
                                      if (!mounted || picked == null) return;
                                      setState(() {
                                  ...
                                  const SizedBox(height: AppTheme.spaceS),
                                  _chipWrap(
                                    _selectedOfferingAssets.map((item) => item.name).toList(growable: false),
                                    color: AppTheme.neonPink,
                                  ),
                                  const SizedBox(height: AppTheme.spaceM),
                                  Text('Seed Bundles', style: AppTheme.tradingHeading(fontSize: 18, color: AppTheme.neonCyan)),
                                  const SizedBox(height: 8),
                                  _bundleRow(
                                    label: '10 Seeds',
                                    value: _smallBundles,
                                    onMinus: () => setState(() => _smallBundles = (_smallBundles - 1).clamp(0, 999)),
                                    onPlus: () => setState(() => _smallBundles += 1),
                                  ),
                                  _bundleRow(
                                    label: '50 Seeds',
                                    value: _mediumBundles,
                                    onMinus: () => setState(() => _mediumBundles = (_mediumBundles - 1).clamp(0, 999)),
                                    onPlus: () => setState(() => _mediumBundles += 1),
                                  ),
                                  _bundleRow(
                                    label: '100 Seeds',
                                    value: _largeBundles,
                                    onMinus: () => setState(() => _largeBundles = (_largeBundles - 1).clamp(0, 999)),
                                    onPlus: () => setState(() => _largeBundles += 1),
                                  ),
                                  const SizedBox(height: 8),
                                  Text('Total offered seed value: $_seedTotal', style: TextStyle(color: AppTheme.tradingMutedText)),
                                ],
                              ),
                            ),
                            _sectionCard(
                              title: 'What You Want Back',
                              subtitle: 'Ask for specific blueprints and assets, or open the door to custom offers.',
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  SwitchListTile(
                                    ...
                                    title: const Text('Open to offers', style: TextStyle(color: Colors.white)),
                                    subtitle: const Text(
                                  ...
                                  _selectorTile(
                                    ...
                                    onTap: _openToOffers || _wantsNothing || missingBlueprints.isEmpty
                                        ...
                                        : () async {
                                            final picked = await _showBlueprintMultiPicker(
                                              title: 'Select wanted blueprints',
                                              items: missingBlueprints,
                                              initiallySelected: _selectedWantedBlueprints,
                                            );
                                            if (!mounted || picked == null) return;
                                            setState(() {
                                  ...
                                  const SizedBox(height: AppTheme.spaceS),
                                  _chipWrap(_selectedWantedBlueprints.map((item) => item.name).toList(growable: false)),
                                  const SizedBox(height: AppTheme.spaceM),
                                  _selectorTile(
                                    ...
                                    onTap: _openToOffers || _wantsNothing
                                        ...
                                        : () async {
                                            final picked = await _showAssetMultiPicker(
                                              title: 'Select wanted trade assets',
                                              initiallySelected: _selectedWantedAssets,
                                            );
                                            if (!mounted || picked == null) return;
                                            setState(() {
                                  ...
                                  const SizedBox(height: AppTheme.spaceS),
                                  _chipWrap(
                                    _selectedWantedAssets.map((item) => item.name).toList(growable: false),
                                    color: AppTheme.neonPink,
                                  ),
                                ],
                              ),
                            ...
                            _sectionCard(
                              ...
                              subtitle: 'Use both toggles if you want the listing to prefer a full batch while still allowing people to pitch custom or partial make-up offers.',
                              child: Column(
                                children: [
                                  SwitchListTile(
                                    ...
                                    activeThumbColor: AppTheme.neonPink,
                                    contentPadding: EdgeInsets.zero,
                                    title: const Text('Trade as one batch / bundle', style: TextStyle(color: Colors.white)),
                                    subtitle: const Text(
                                  ...
                                  SwitchListTile(
                                    ...
                                    onChanged: (value) =>
                                        setState(() => _allowPartialOffers = value),
                                    activeThumbColor: AppTheme.neonPink,
                                    contentPadding: EdgeInsets.zero,
                                    title: const Text('Allow partial / custom offers', style: TextStyle(color: Colors.white)),
                                    subtitle: Text(
                                      _tradeAsBundle
                              ...
                              child: Column(
                                children: [
                                  CheckboxListTile(
                                    ...
                                    onChanged: (value) => setState(() => _acceptsBlueprints = value ?? false),
                                    title: const Text('Accept blueprints', style: TextStyle(color: Colors.white)),
                                    controlAffinity: ListTileControlAffinity.leading,
                                    activeColor: AppTheme.neonPink,
                                  ),
                                  CheckboxListTile(
                                    value: _acceptsSeeds,
                                    onChanged: (value) => setState(() => _acceptsSeeds = value ?? false),
                                    title: const Text('Accept seeds', style: TextStyle(color: Colors.white)),
                                    controlAffinity: ListTileControlAffinity.leading,
                                    activeColor: AppTheme.neonPink,
                                  ),
                                  CheckboxListTile(
                                    value: _acceptsResources,
                                    onChanged: (value) => setState(() => _acceptsResources = value ?? false),
                                    title: const Text('Accept resources / keys / weapons / mods', style: TextStyle(color: Colors.white)),
                                    controlAffinity: ListTileControlAffinity.leading,
                                    activeColor: AppTheme.neonPink,
                                  ),
                                  SwitchListTile(
                                    value: _seriousOffersOnly,
                                    onChanged: (value) => setState(() => _seriousOffersOnly = value),
                                    activeThumbColor: AppTheme.neonPink,
                                    contentPadding: EdgeInsets.zero,
                                    title: const Text('Serious offers only', style: TextStyle(color: Colors.white)),
                                  ),
                                  _buildDropdown(
                                    label: 'Best Play Window',
                                    value: _selectedPlayWindow,
                                    options: const ['Flexible', 'Afternoons', 'Evenings', 'Late Night', 'Weekends'],
                                    onChanged: (value) => setState(() => _selectedPlayWindow = value ?? 'Evenings'),
                                  ),
                                  const SizedBox(height: AppTheme.spaceM),
                                  _buildDropdown(
                                    label: 'Listing Expiry',
                                    value: _selectedExpiry,
                                    options: const ['24 Hours', '72 Hours', '7 Days'],
                                    onChanged: (value) => setState(() => _selectedExpiry = value ?? '72 Hours'),
                                  ),
                                ],
                              ),
                            ),
                            _sectionCard(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text('Title: ${_buildTitle()}', style: const TextStyle(color: Colors.white)),
                                  const SizedBox(height: 8),
                                  Text('Offering: ${_buildOfferSummary().isEmpty ? 'Nothing selected yet' : _buildOfferSummary()}', style: TextStyle(color: AppTheme.tradingMutedText)),
                                  const SizedBox(height: 8),
                                  Text('Looking for: ${_buildWantedSummary().isEmpty ? 'Nothing selected yet' : _buildWantedSummary()}', style: TextStyle(color: AppTheme.tradingMutedText)),
                                  const SizedBox(height: 8),
                                  Text(
                            ...
                            _sectionCard(
                              ...
                              child: TextFormField(
                                ...
                                decoration: AppTheme.tradingInputDecoration(
                                  label: 'Optional details, preferred swap setup, level 4 only, exact ratios, etc.',
                                ),
                              ),
                            ),
                            ElevatedButton(
