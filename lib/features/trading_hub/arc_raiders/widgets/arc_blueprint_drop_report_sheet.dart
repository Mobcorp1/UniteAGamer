...
class _ArcBlueprintDropReportSheetState
  ...
  Future<T?> _showSearchPicker<T>({
    ...
    final sortedItems = List<T>.from(items)
      ..sort(
        (a, b) =>
            labelBuilder(a).toLowerCase().compareTo(labelBuilder(b).toLowerCase()),
      );
              ...
              setModalState(() {
                if (normalized.isEmpty) {
                  ...
                  filteredItems = sortedItems
                      .where(
                        (item) =>
                            labelBuilder(item).toLowerCase().contains(normalized),
                      )
  ...
  Future<void> _pickEntryTime() async {
    final selection = await _showSearchPicker<ArcEntryTime>(
      title: 'Entry Time',
      items: ArcEntryTime.values,
      labelBuilder: (item) => item.label,
    );
    if (selection == null || !mounted) return;
    setState(() => _entryTime = selection);
    _scrollToNextStep();
  }
  ...
  Future<void> _pickAdditionalBlueprint(int index) async {
    ...
    final items = List<ArcBlueprint>.from(ArcBlueprintSeedData.blueprints)
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '${widget.blueprint.category} • ${widget.blueprint.group} • ${widget.blueprint.rarityLabel}',
                    style: Theme.of(
                      context,
                  ...
                  const Text(
                    'Fill out the run in this order: map, map event, area / POI, container type, raid round, entry time, then time of day. Add as many extra blueprints from the same raid as you need below.',
                  ...
                  _buildSelectorField(
                    label: 'Entry Time *',
                    value: _entryTime?.label ?? 'Select Entry Time',
                    onTap: _selectedMap == null ? null : _pickEntryTime,
                  ),
                  const SizedBox(height: AppTheme.spaceM),
                  _buildSelectorField(
                    label: 'Time of Day *',
                    value: _timeOfDay?.label ?? 'Select Time of Day',
                  ...
                  Row(
                    children: [
                      ...
                      TextButton.icon(
                        onPressed:
                            _selectedMap == null ? null : _addAnotherBlueprintEntry,
